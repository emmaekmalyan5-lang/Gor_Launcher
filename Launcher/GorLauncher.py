...
class GORLauncher(QMainWindow):
    ...
    def init_ui(self):
        self.setStyleSheet("""
            ...
            QLineEdit { background: #1a1a1a; border-radius: 15px; padding: 8px 20px; border: 1px solid #333; color: white; }
            QComboBox { background: #1a1a1a; padding: 5px; border-radius: 5px; }
        """)
    ...
    def add_game_dialog(self):
        dlg = GameEditor(self)
        if dlg.exec(): self.load_data(); self.refresh_list(); self.update_stats()
    ...
    def add_group(self):
        ...
        if dlg.exec(): self.load_data(); self.refresh_list()
