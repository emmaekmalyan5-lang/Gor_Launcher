...
class HistoryCard(QFrame):
    ...
    def init_ui(self):
        ...
        # Загрузка обложки для истории
        icon_path = self.entry.get('icon', '')
...
class GameEditDialog(QDialog):
    def __init__(self, parent=None, game_data=None, groups=None, current_group=None):
        ...
        self.game_data = game_data or {"name": "", "path": "", "icon": "", "group": "Без группы", "args": "", "playtime_seconds": 0, "favorite": False}
...
class GameCard(QFrame):
    ...
    def init_ui(self):
        ...
        # ЗАГРУЗКА ОБЛОЖКИ (с проверкой пути)
        icon_path = self.game_data.get('icon', '')
...
class GORLauncher(QMainWindow):
    def __init__(self):
        ...
        # Пути к Sunshine (в папке проекта)
    ...
    def sync_games_with_sunshine(self):
        try:
            # 1. Собираем все игры из нашего лаунчера
            all_apps = []
            
            # Добавляем стандартные (Desktop/Steam), если хочешь, или только свои:
            # all_apps.append({"name": "Desktop", "image-path": "desktop.png"})
            
            # Проходим по всем играм в лаунчере
            temp_list = self.games_info["standalone"][:]
            for grp in self.games_info["groups"].values():
                temp_list.extend(grp)
                
            for g in temp_list:
                cmd_str = g["path"]
                if g.get("args"):
                    # Оборачиваем путь в кавычки если есть пробелы + добавляем аргументы
                    cmd_str = f'"{g["path"]}" {g["args"]}'
                
                app_entry = {
                    "name": g["name"],
                    "cmd": cmd_str,
                    "working-dir": os.path.dirname(g["path"]),
                    "auto-detach": True
                }
                all_apps.append(app_entry)
            
            # 2. Формируем финальный JSON для Sunshine
            sunshine_config = {
                "env": {},
                "apps": all_apps
            }
            
            # 3. Сохраняем файл apps.json
            os.makedirs(os.path.dirname(self.apps_json_path), exist_ok=True)
            with open(self.apps_json_path, 'w', encoding='utf-8') as f:
                json.dump(sunshine_config, f, indent=4, ensure_ascii=False)
            
            # 4. Перезапуск Sunshine
            # Пытаемся убить процесс, если он запущен
            try: os.system("taskkill /f /im sunshine.exe")
            except: pass
            
            time.sleep(1.5) # Пауза чтобы файл освободился
            
            if os.path.exists(self.sunshine_exe):
                subprocess.Popen([self.sunshine_exe], cwd=self.sunshine_dir)
                QMessageBox.information(self, "Синхронизация", "Список игр в Sunshine обновлен и сервер перезапущен!")
            else:
                QMessageBox.warning(self, "Ошибка", f"Файл {self.sunshine_exe} не найден!")

            # ОЧЕНЬ ВАЖНО: После синхронизации обновляем интерфейс лаунчера, чтобы обложки не пропали
            self.refresh_list()
    ...
    def load_data(self):
        if os.path.exists(self.data_file):
            try:
                with open(self.data_file, 'r', encoding='utf-8') as f: self.games_info = json.load(f)
            except: self.games_info = {"groups": {}, "standalone": [], "history": []}
        else: self.games_info = {"groups": {}, "standalone": [], "history": []}
        if "history" not in self.games_info: self.games_info["history"] = []

    def save_data(self):
        with open(self.data_file, 'w', encoding='utf-8') as f: json.dump(self.games_info, f, indent=4)
    ...
    def init_ui(self):
        ...
        btn_sync = QPushButton("🔄 СИНХРОНИЗИРОВАТЬ SUNSHINE"); btn_sync.clicked.connect(self.sync_games_with_sunshine)
        tool_lay.addWidget(btn_add); tool_lay.addWidget(btn_grp); tool_lay.addWidget(btn_sync); tool_lay.addStretch()
        self.lib_lay.addLayout(tool_lay)
        self.scroll_lib = QScrollArea(); self.scroll_lib.setWidgetResizable(True); self.scroll_lib.setStyleSheet("border:none;")
        ...
        self.scroll_fav = QScrollArea(); self.scroll_fav.setWidgetResizable(True); self.scroll_fav.setStyleSheet("border:none;")
        ...
        self.scroll_hist = QScrollArea(); self.scroll_hist.setWidgetResizable(True); self.scroll_hist.setStyleSheet("border:none;")
    ...
    def add_game_dialog(self):
        ...
        if dlg.exec():
            d = dlg.get_data(); t = d.pop('group')
            if t == "Без группы": self.games_info["standalone"].append(d)
            else: self.games_info["groups"][t].append(d)
            self.save_data(); self.refresh_list(); self.update_stats()
    ...
    def edit_game(self, old, grp):
        ...
        if dlg.exec():
            ...
            if grp and grp in self.games_info["groups"]: self.games_info["groups"][grp].append(old)
            else: self.games_info["standalone"].append(old)
        self.save_data(); self.refresh_list(); self.update_stats()
    ...
    def add_group(self):
        ...
        if ok and n and n not in self.games_info["groups"]:
            self.games_info["groups"][n] = []; self.save_data(); self.refresh_list()
