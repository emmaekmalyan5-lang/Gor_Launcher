...
# --- ИСПРАВЛЕННАЯ ЛОГИКА ПОДКЛЮЧЕНИЯ БРАУЗЕРА ---
current_dir = os.path.dirname(os.path.abspath(__file__))
# Путь к папке src внутри скачанного браузера
browser_src_path = os.path.join(current_dir, "Brauzer", "src")

if os.path.exists(browser_src_path):
    if browser_src_path not in sys.path:
        sys.path.append(browser_src_path)
    try:
        # В скачанном проекте главный файл обычно main.py
        # Импортируем MainWindow (или Browser) из main.py
        from main import MainWindow as GORBrowser 
    except (ImportError, ModuleNotFoundError) as e:
        print(f"Ошибка импорта браузера: {e}")
        GORBrowser = None
else:
    GORBrowser = None
...
class GORLauncher(QMainWindow):
    ...
    def init_ui(self):
        ...
        header.addWidget(title); header.addStretch(); header.addWidget(self.stats_lbl); header.addWidget(self.search_bar)
        ...
        # 4. ВКЛАДКА ИНТЕРНЕТ (Интеграция GORBrowser)
        if GORBrowser:
            try:
                # В современных браузерах на PyQt6 конструктор часто требует аргументы или специальный вызов
                self.browser_tab = GORBrowser()
                self.tabs.addTab(self.browser_tab, "🌐 ИНТЕРНЕТ")
            except Exception as e:
                self.error_tab = QWidget()
                err_lay = QVBoxLayout(self.error_tab)
                err_lay.addWidget(QLabel(f"Ошибка запуска браузера: {e}"), alignment=Qt.AlignmentFlag.AlignCenter)
                self.tabs.addTab(self.error_tab, "🌐 ИНТЕРНЕТ (ОШИБКА)")
        else:
            self.error_tab = QWidget()
            err_lay = QVBoxLayout(self.error_tab)
            err_lay.addWidget(QLabel("Браузер не найден. Проверьте папку Brauzer/src/main.py"), alignment=Qt.AlignmentFlag.AlignCenter)
            self.tabs.addTab(self.error_tab, "🌐 ИНТЕРНЕТ (ОШИБКА)")
